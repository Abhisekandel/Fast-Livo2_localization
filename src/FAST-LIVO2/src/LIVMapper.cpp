...
void LIVMapper::readParameters(rclcpp::Node::SharedPtr &nh)
{
  ...
  voxeloctree_file_path = nh->declare_parameter<string>("voxeloctree_file_path", "");

  p_pre->blind_sqr = p_pre->blind * p_pre->blind;

  if (extrinT.size() != 3)
    throw std::runtime_error("Parameter 'extrin_calib.extrinsic_T' must contain exactly 3 values.");

  if (extrinR.size() != 9)
    throw std::runtime_error("Parameter 'extrin_calib.extrinsic_R' must contain exactly 9 values.");

  if (cameraextrinT.size() != 3)
    throw std::runtime_error("Parameter 'extrin_calib.Pcl' must contain exactly 3 values.");

  if (cameraextrinR.size() != 9)
  {
    auto Rlc = nh->declare_parameter<vector<double>>("extrin_calib.Rlc", vector<double>());
    if (Rlc.size() == 9)
    {
      cameraextrinR = Rlc;
      RCLCPP_WARN(nh->get_logger(),
                  "Parameter 'extrin_calib.Rcl' missing; using 'extrin_calib.Rlc'. Please update the configuration file.");
    }
    else
    {
      throw std::runtime_error("Parameter 'extrin_calib.Rcl' must contain exactly 9 values.");
    }
  }
}
...
void LIVMapper::initializeSubscribersAndPublishers(rclcpp::Node::SharedPtr &nh, image_transport::ImageTransport &it)
{
  ...
  pubMapVisualization = nh->create_publisher<visualization_msgs::msg::MarkerArray>("/map_visualization", 1);
  if (!voxeloctree_file_path.empty()) {
    map_pub_timer = nh->create_wall_timer(
        std::chrono::seconds(10),
        std::bind(&LIVMapper::publishMapVisualization, this)
    );
  }
}
...
cv::Mat LIVMapper::getImageFromMsg(const sensor_msgs::msg::Image::ConstPtr &img_msg)
{
  ...
  if (img_msg->encoding == "bayer_gbrg8") {
    // 处理 Bayer 格式
    cv::Mat bayer_img = cv_bridge::toCvCopy(img_msg, "bayer_gbrg8")->image;
    cv::cvtColor(bayer_img, img, cv::COLOR_BayerGB2BGR);
    std::cout << "Converted Bayer image to BGR format." << std::endl;
  } else {
    // 原有的 BGR8 处理
...
std::vector<FastLioVoxel> LIVMapper::loadVoxelOctree(const std::string& file_path) {
    std::vector<FastLioVoxel> map_data;
    std::ifstream infile(file_path, std::ios::binary);
    if (!infile.is_open()) {
        std::cerr << "Error: Could not open voxel map file: " << file_path << std::endl;
        return map_data;
    }

    uint32_t point_count = 0;
    infile.read(reinterpret_cast<char*>(&point_count), sizeof(uint32_t));
    map_data.resize(point_count);
    infile.read(reinterpret_cast<char*>(map_data.data()), point_count * sizeof(FastLioVoxel));
    infile.close();
    return map_data;
}
...
void LIVMapper::loadPrebuiltMap(const std::string& file_path) {
    ...
    double planer_threshold = voxelmap_manager->config_setting_.planner_threshold_;
    ...
    for (const auto& voxel : map_data) {
        ...
        if (std::abs(normal(2)) < 0.9) {
            ...
            tangent1 = normal.cross(Eigen::Vector3d(1, 0, 0)).normalized();
        }
