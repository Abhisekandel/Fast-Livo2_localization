...
struct FastLioVoxel {
    float center[3];     // 12 bytes: x, y, z
    float normal[3];     // 12 bytes: nx, ny, nz
    float plane_d;       // 4 bytes:  Hesse normal form scalar (d)
    uint32_t color_rgba; // 4 bytes:  Packed color
};

class LIVMapper
{
public:
  std::vector<FastLioVoxel> loadVoxelOctree(const std::string& file_path);
  void loadPrebuiltMap(const std::string& file_path);
...
public:
  ...
  rclcpp::Subscription<livox_ros_driver2::msg::CustomMsg>::SharedPtr sub_lv_pcl;
